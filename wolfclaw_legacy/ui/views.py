...
from core.orchestrator import orchestrate_task
# ─────────────────────────── LOGIN ───────────────────────────
...
def war_room_view():
    st.header("🦅 War Room (Multi-Agent Orchestration)")
    st.info(
        "**What is this page?**\n\n"
        "The War Room lets you assign a complex task to a team of AI bots. You select a **Manager Bot** "
        "and one or more **Sub-Bots**. The Manager breaks the task down, delegates sub-tasks to the Sub-Bots, "
        "collects their results, and writes a final summary."
    )
    
    bots = get_bots()
    if not bots:
        st.warning("⚠️ You need to create at least two bots first! Go to **Manage Bots** in the sidebar.")
        return
        
    bot_names = {b_id: b_data["name"] + f" ({b_data['model']})" for b_id, b_data in bots.items()}
    bot_ids = list(bot_names.keys())
    
    col1, col2 = st.columns(2)
    with col1:
        st.subheader("1. Setup the Team")
        manager_id = st.selectbox("Select Manager Bot", options=bot_ids, format_func=lambda x: bot_names[x], help="This bot leads the mission and delegates tasks.")
        
        # Filter out manager from sub-bot options
        sub_bot_options = [b for b in bot_ids if b != manager_id]
        if not sub_bot_options:
             st.warning("You need at least one other bot to act as a Sub-Bot.")
             return
             
        sub_bot_ids = st.multiselect("Select Sub-Bots", options=sub_bot_options, format_func=lambda x: bot_names[x], help="These bots execute the delegated sub-tasks.")
        
    with col2:
        st.subheader("2. Define the Mission")
        mission_prompt = st.text_area("Task Description", placeholder="e.g., Research quantum computing advancements in 2024 and write a summary.", height=150)
        
    if st.button("🚀 Start War Room Mission", type="primary"):
        if not sub_bot_ids:
            st.error("Please select at least one Sub-Bot.")
            return
        ...
        with st.spinner(f"{bots[manager_id]['name']} is planning the mission..."):
             try:
                 messages = [{"role": "user", "content": mission_prompt}]
                 # Use a generator to consume the orchestrator events
                 for event in orchestrate_task(manager_id, sub_bot_ids, messages):
                     with feed_container:
                         if event["type"] == "status":
                             st.markdown(f"**⚡ {event['bot_name']}:** _{event['content']}_")
                         elif event["type"] == "message":
                             with st.chat_message("assistant", avatar="🤖"):
                                 st.markdown(f"**{event['bot_name']}**:\n\n{event['content']}")
                         elif event["type"] == "error":
                             st.error(f"Error from {event['bot_name']}: {event['content']}")
             except Exception as e:
...
def integrations_view():
    st.header("🧩 App Integrations")
    st.info("Connect Wolfclaw to external services to give your AI assistants real-world capabilities.")
    
    col1, col2 = st.columns(2)
    
    with col1:
        st.subheader("Google Workspace")
        st.caption("Read Gmail and check Google Calendar.")
        st.markdown("**Upload credentials.json**")
        st.markdown("Get this file from your Google Cloud Console. Needed for OAuth.")
        
        uploaded_file = st.file_uploader("Upload credentials.json", type=['json'], key="google_creds_upload")
        if uploaded_file is not None:
            try:
                import json
                ...
                # Save to core/integrations/google_credentials.json (relative to root)
                # Ensure the directory exists
                target_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "core", "integrations")
                os.makedirs(target_dir, exist_ok=True)
                
                target_file = os.path.join(target_dir, "google_credentials.json")
                with open(target_file, "w") as f:
                    json.dump(creds_data, f)
                    
                st.success("✅ credentials.json saved successfully! The app will prompt for OAuth login next time a bot tries to use Google tools.")
            except Exception as e:
                st.error(f"Failed to save credentials: {e}")
                
        # Check status
        target_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "core", "integrations")
        if os.path.exists(os.path.join(target_dir, "google_token.json")):
            st.success("Google is Authenticated & Ready!")
        elif os.path.exists(os.path.join(target_dir, "google_credentials.json")):
            st.warning("Credentials found. Needs Auth (Try using a Google tool in Chat).")
        else:
            st.error("Not Configured")
            
    with col2:
        st.subheader("Slack")
        st.caption("Post messages and read channels.")
        st.markdown("**Bot OAuth Token**")
        
        current_slack_token = os.environ.get("SLACK_BOT_TOKEN") or get_key("slack") or ""
        slack_token = st.text_input("Bot Token (xoxb-)", value=current_slack_token, type="password")
        
        if st.button("💾 Save Slack Token"):
            if slack_token:
                os.environ["SLACK_BOT_TOKEN"] = slack_token
                try:
                    set_key("slack", slack_token)
                except:
                    pass
                st.success("✅ Slack token saved successfully!")
                st.rerun()
                
        if current_slack_token:
            st.success("Slack is Configured & Ready!")
        else:
            st.error("Not Configured")
...
def macros_view():
    st.header("🎥 Watch & Learn Macros")
    st.info("Record your actions (clicks & keystrokes) and Wolfclaw will use Vision AI to build an automated workflow.")
    ...
    with col1:
        ...
        is_recording = st.session_state.get("is_recording_macro", False)
        ...
        if not is_recording:
            if st.button("🔴 Start Recording", type="primary"):
                try:
                    ...
                    st.session_state["is_recording_macro"] = True
                    st.session_state["macro_session_id"] = recorder.session_id
                    st.success(res)
                    st.rerun()
                except Exception as e:
                    st.error(f"Failed to start: {e}")
        else:
            if st.button("⏹️ Stop & Analyze", type="primary"):
                try:
                    from core.macro_recorder import recorder
                    from core.vision_analyzer import analyze_macro_session
                    session_id = st.session_state.get("macro_session_id")
                    res = recorder.stop_recording()
                    st.session_state["is_recording_macro"] = False
                    
                    st.info("Analyzing with Vision AI... this may take a moment.")
                    # Run analysis
                    result = analyze_macro_session(session_id)
                    if "error" in result:
                        st.error(result["error"])
                    else:
                        st.session_state["last_macro_result"] = result
    ...
    with col2:
        st.subheader("Generated Automation (Flow JSON)")
        last_result = st.session_state.get("last_macro_result")
        
        if last_result:
            st.json(last_result)
            st.caption("You can import this JSON into the Flows UI on the Web Dashboard.")
        else:
            st.caption("No macro recorded yet in this session.")

# ─────────────────────────── PLUGIN MARKETPLACE ───────────────────────────

def marketplace_view():
    st.header("🏪 Plugin Store")
    st.info("Extend Wolfclaw's capabilities with community-built Python tools.")
    
    st.subheader("Available Plugins")
    
    try:
        import requests
        # Port 8000 is default for FastAPI. Adjust if different.
        API_BASE = "http://localhost:8000/api"
        
        try:
            store_res = requests.get(f"{API_BASE}/marketplace/")
            inst_res = requests.get(f"{API_BASE}/marketplace/installed")
            store_data = store_res.json()
            inst_data = inst_res.json()
        except Exception:
            # Fallback if FastAPI is not running alongside Streamlit
            st.warning("Could not connect to the Backend API. Make sure the main Wolfclaw backend is running.")
            return
            
        plugins = store_data.get("plugins", [])
        installed = inst_data.get("installed", [])
        ...
        for p in plugins:
            with st.container():
                ...
                with col1:
                    is_inst = "✅ Installed" if p["id"] in installed else ""
                    st.markdown(f"#### {p['name']} {is_inst}")
                    st.caption(f"by {p['author']} • {p['downloads']} downloads")
                    st.write(p["description"])
                with col2:
                    if p["id"] in installed:
                        if st.button("🗑️ Uninstall", key=f"uninst_{p['id']}"):
                            res = requests.post(f"{API_BASE}/marketplace/uninstall/{p['id']}")
                        ...
                        if st.button("⬇️ Install", type="primary", key=f"inst_{p['id']}"):
                            res = requests.post(f"{API_BASE}/marketplace/install/{p['id']}")
                            if res.status_code == 200:
...
def dashboard_view():
    ...
    if "_nav_override" in st.session_state:
        ...
        if override_val in ["Chat", "War Room", "Manage Bots", "Bot Profiles", "Deploy Channels", "Integrations", "Macros", "Plugin Store", "Remote Servers", "Settings", "Logout"]:
            st.session_state["nav_radio"] = override_val
            
    menu_options = ["Chat", "War Room", "Manage Bots", "Bot Profiles", "Deploy Channels", "Integrations", "Macros", "Plugin Store", "Remote Servers", "Settings", "Logout"]
