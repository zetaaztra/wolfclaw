...
async function deleteBot(id) {
    ...
    const resp = await fetch(`${API_BASE}/bots/${id}`, { method: 'DELETE' });
    if (resp.ok) loadBots();
}
...
async function sendMessage() {
    ...
    try {
        ...
        if (resp.ok) {
            ...
            appendChatBubble('assistant', `⚠️ Error: ${data.detail || 'Unknown error'}`);
        }
    } catch (err) {
        const thinkingEl = document.getElementById(thinkingId);
        if (thinkingEl) thinkingEl.remove();
        appendChatBubble('assistant', '⚠️ Connection error. Is the backend running?');
    }
}
...
async function deleteBot(targetId = null) {
    ...
    if (!confirm("⚠️ WARNING: This will permanently delete this bot and ALL of its chat history, knowledge base documents, and scheduled tasks. This cannot be undone.\n\nAre you sure?")) {
        return;
    }
    ...
    try {
        ...
        if (resp.ok) {
            ...
            loadBots();
            loadProfileBots();
            loadKnowledgeBots(); // Ensure this exists or catch
...
function getBlockConfigHTML(blockType) {
    switch (blockType) {
        case 'ai_prompt':
            return `
                <input type="text" class="df-input" df-model placeholder="Model (gpt-4o)" style="margin-bottom:4px;">
