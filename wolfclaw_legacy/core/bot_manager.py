...
def _get_active_workspace_id() -> str:
    ...
    if os.environ.get("WOLFCLAW_ENVIRONMENT") == "desktop":
        ...
        return local_db.create_workspace(user_id, "Default Workspace")
...
def save_bot(bot_id: str, name: str, model: str, prompt: str, fallback_models: list = None):
    """Save a bot profile to Supabase or Local DB."""
    workspace_id = _get_active_workspace_id()
    
    if os.environ.get("WOLFCLAW_ENVIRONMENT") == "desktop":
        from core import local_db
        local_db.create_bot(workspace_id, name, model, prompt, fallback_models)
        return
...
def read_workspace_file(bot_id: str, filename: str) -> str:
    ...
    if os.environ.get("WOLFCLAW_ENVIRONMENT") == "desktop":
        ...
        bot = bots.get(bot_id)
        if bot:
            if filename == "SOUL.md": return bot["prompt"]
        return ""
...
def write_workspace_file(bot_id: str, filename: str, content: str):
